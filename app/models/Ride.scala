...
object Ride {
  var rides = List(Ride(1,"Liza","London","Manchester",false,12),
    ...
    Ride(3,"Henderson","London","New Castle",false,15),
    Ride(4,"Alex","London","Manchester",true,12)
  )
  def findAll = rides.sortBy(_.id)
  def findByParams(fromCity: String,toCity:String) = rides.filter(_.fromCity == fromCity).filter(_.toCity == toCity).toList
}
